    ...
    task check_and_print_regs;
        ...
        begin
            ...
            for (i = 0; i < 8; i = i + 1) begin
                if (uut.registers[i] !== expected_regs[i]) begin
                    $display("      R%0d: Expected=%h, Actual=%h  <-- MISMATCH", i, expected_regs[i], uut.registers[i]);
                    errors = errors + 1;
                end else begin
                    $display("      R%0d: Expected=%h, Actual=%h", i, expected_regs[i], uut.registers[i]);
                end
            end
